...
- Reject known ASR hallucinations / non-speech artefacts ("субтитры сделал…", "продолжение следует", "music")
...
When `SOUNDSIGHT_ENABLE_LLM=true` is set, `core/pipeline.py` runs an optional post-routing stage via `agent/llm_refiner.py` on every accepted segment:

- Rewrites raw ASR output into clean accessible text (segments ≤20 words)
- Condenses long speech blocks (>20 words) into 1-2 sentences

The refined text is what reaches subtitles, avatar, and the session transcript.
Structured bullet-point note generation (`generate_structured_note`) is also
available in the refiner module and reserved for future note export.

Any OpenAI-compatible API works (OpenAI, local llama.cpp, Ollama):

```env
SOUNDSIGHT_ENABLE_LLM=true
SOUNDSIGHT_LLM_PROVIDER=openai_compatible
SOUNDSIGHT_LLM_MODEL=gpt-4o-mini
SOUNDSIGHT_LLM_BASE_URL=https://api.openai.com/v1
SOUNDSIGHT_LLM_API_KEY=sk-...
SOUNDSIGHT_LLM_TIMEOUT=5.0
```
...
| KZ model (fine-tuned) | abilmansplus/whisper-turbo-ksc2 |
| Speech gating | RMS energy gate + streaming endpointing on trailing silence |
...
**Technical depth** – multi-stage pipeline: RMS gate → streaming endpointing (trailing-silence finalisation) → Whisper ASR → keyword extraction → agent policy scoring → conditional avatar synthesis + conditional session storage. Each stage has a clear, bounded responsibility.
