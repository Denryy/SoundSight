"""
SoundSight Agent Benchmark — baseline vs agent mode comparison.
...
def _run_agent(
    ...
    for case in cases:
        ...
        sum_match = decision.include_in_summary == case.expect_summary
...
def _markdown_table(results: list[CaseResult], stats: BenchmarkStats) -> str:
    lines = []
    lines.append("# SoundSight Agent Evaluation Report")
    ...
    lines.append(
        f"The SoundSight agent rejected {noise_pct}% of segments as noise/filler and suppressed "
...
def main() -> None:
    # Windows consoles default to a non-UTF-8 codepage; force UTF-8 so the
    # markdown (which contains "✓") prints instead of raising UnicodeEncodeError.
    try:
    ...
    # Save reports first so a console-encoding hiccup never loses the output.
