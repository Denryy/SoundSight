...
# ---------------------------------------------------------------------------
# Stopwords (Russian + English) — shared single source of truth
# ---------------------------------------------------------------------------
...
class AgentPolicy:
    ...
    def evaluate_segment(
        ...
        if self._is_hallucination(text):
            decision.is_noise = True
            decision.reason = "ASR hallucination / non-speech artefact"
            decision.reasons = ["HALLUCINATION"]
            return decision
    ...
    def _is_hallucination(text: str) -> bool:
        """
        Detect known Whisper hallucinations / non-speech artefacts that survive
        the filler gates (multi-word, real-looking phrases like "субтитры сделал").
        Operates on the raw ASR text before cleaning.
        """
        t = text.strip().lower().strip(".,!?-– …[]()\"'")
        if t in HALLUCINATION_EXACT:
            return True
        return any(s in t for s in HALLUCINATION_SUBSTR)
    ...
    def _compute_importance(
        ...
        # Sentence completeness bonus (raw text ends with . ! ?).
        # Must use raw_text — cleaned text has punctuation stripped.
        sentence_complete = bool(re.search(r"[.!?]\s*$", raw_text.strip()))
        sentence_bonus = 1.0 if sentence_complete else 0.0
