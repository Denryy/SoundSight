# SoundSight — Жюриге арналған архитектура сипаттамасы

## Жүйенің мақсаты

SoundSight — есту қабілеті нашар пайдаланушыларға арналған autonomous accessibility agent. Тікелей дауыс ағынын қабылдап, бірнеше қолжетімді форматта шығаруды автоматты түрде басқарады.
...
Браузер (микрофон)
        │
        │ WebSocket /ws/subtitles (binary SoundSight protocol)
        ▼
┌─────────────────────────────────────────────────┐
│              core/pipeline.py                   │
│                                                 │
│  1. Frame parsing (SoundSight header / raw PCM)        │
│  2. Resampling → 16 kHz                         │
...
╚═════════════════════════════════════════════════╝
  ...
  JSON payload   translate        append
                 ...
                 CWASA render
                      │
                      ▼ (optional, if SOUNDSIGHT_ENABLE_LLM=true)
...
- `SOUNDSIGHT_ENABLE_LLM=true` болса ғана белсенді
